...
continous.df <- as.data.frame(continous.inventory(4))
row.names(period.df)<- c("yt","qt","lt")

#[,1:11]
kable(continous.df,caption = "Continous Review Inventory  Policy", row.names = T, escape = F,format = "pandoc")
```
...
```{r}
q.year.vec <- vector(mode ="double" ,length = 62)

for(item in 1:62){
  q.year.vec[item] <- sum(continous.inventory(item)[2,])/ product_data$cycle_time_in_days[item] #convert back to m 
}
