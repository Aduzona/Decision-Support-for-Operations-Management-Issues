...
qopt <- OP(
  objective = F_objective(F=function(q) ((1500*62)+sum((dei/q)*cori)+ (h*sum((q/2)*box_cost)))
,n=length(product_data$`material ID`)),
types = rep("C",length(product_data$`material ID`)),
bounds = V_bound(ub= product_data$eoq,lb= rep(1, length(product_data$`material ID`))),
constraints = F_constraint(F=function(q) q*((b_not_sorting*b_sorting)/rack_length),
dir="<=",
rhs = rack_total_width)
)

c(numeric(length(product_data$`material ID`)))
?ROI_solve()
#solve the problem
sol.const <- ROI_solve(qopt,start=c(numeric(length(product_data$`material ID`))),solver = "alabama")
