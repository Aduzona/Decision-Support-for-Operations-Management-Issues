...
M= 32 levels,K= 9 patterns $x_{m,k}$  consist of a vector of decision variables with $(M \cdot K)$ 288 elements $X= ( K_1-M1,K2-M1,..K9-M1,....K1-M32,K2-M32,..K9-M32)$. $J=3$ unique lane width.There are $ J+K= 33$ constraints. Thus the right hand ride vector $b=(\vec {ld_J},1_1,...1_{288})$. Correspondingly, the constraint matrix $A$ has dimension $291 \times 288$ and looks as follows:

let $J1=200, J2=400, J3=600$

\begin{pmatrix} 
... \\ $ P_{J1}$ \\ $P_{J2}$ \\ $P_{J3}$ \\ 1 \\ 2 \\ ... \\ 288
\end{pmatrix}
\quad
\begin{pmatrix} 
... \\ J1 \\ J2 \\ J3 \\ 1 \\ 2 \\ ... \\ 288
...
b.vec <- c(ld,rep(1,n.m*n.k)) # right hand side vector 

#objective coeficients
c.vec <- c(rep(1,(n.m*n.k)))
  
const.vec <- c(rep(">=",length(ld)), rep("<=",(n.m*n.k))) # vector with constraint directions 

vtype.vec <- rep("B",length(x.vec)) # vector with variable types ("B" binary ) 


A.mat <- matrix(0, ncol = n.m*n.k, nrow = (3+n.m*n.k) )# initialize constraint matrix
...
#sol.levels$solution   # OPTIMAL SOLUTION FOUND shows a matrix of zeros and 1
